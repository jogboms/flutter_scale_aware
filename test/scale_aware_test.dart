...
void main() {
  group('ScaleAware', () {
    ...
    testWidgets('config updates notifies dependents only when changed', (WidgetTester tester) async {
      ...
      final first = ScaleAware(child: builder);
      await tester.pumpWidget(first);

      expect(log, equals(<ScaleAware>[first]));

      final second = ScaleAware(child: builder);
      await tester.pumpWidget(second);

      expect(log, equals(<ScaleAware>[first]));

      final third = ScaleAware(config: ScaleConfig(width: 1, height: 1), child: builder);
      await tester.pumpWidget(third);

      expect(log, equals(<ScaleAware>[first, third]));

      final forth = ScaleAware(child: builder);
